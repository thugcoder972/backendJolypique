...
@SpringBootTest
@AutoConfigureMockMvc
@Disabled // tu peux enlever ça pour exécuter le test
public class AchatControllerTest {
    ...
    @Test
    void createAchat_ShouldReturnCreated() throws Exception {
        // Prépare un DTO conforme à ta nouvelle structure
        AchatDTO dto = new AchatDTO();
        dto.setUser(1L);
        dto.setTicketIds(List.of(10L, 20L)); // Exemple de tickets existants

        // Simule la réponse du service
        Achat achat = new Achat();
        when(achatService.createAchat(any(AchatDTO.class))).thenReturn(achat);
